...
annotationTab.AnnotationElement = function(pvjsElement, xref) {
  this.datasetPreferredPrefix = m.prop(xref.isDataItemIn.preferredPrefix);
  // NOTE: a gpml:GeneProduct is not a biopax:EntityReference, so it
  // cannot be used to specify the xref type.
  this.type = m.prop(pvjsElement.type);
  this.identifier = m.prop(xref.identifier);
  this.textContent = m.prop(pvjsElement.textContent || xref.displayName);
};
//*/
...
annotationTab.vm = (function() {
  ...
  vm.init = function(kaavio) {
    ...
    var annotationElementAndResetSource = kaavio.editor.editorTabsComponent.vm.pvjsElementSource
