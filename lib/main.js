...
var Rx = require('rx');
require('rx-extra/lib/hierarchical-partition.js')(Rx);
...
editor.vm = (function() {
  ...
  vm.init = function(kaavio) {
    ...
    if (!kaavio.sourceData.normalizedPvjsonCurrent) {
      normalizedPvjsonSource = Rx.Observable.zip(
            Rx.Observable.from([kaavio.sourceData.pvjson])
              .flatMap(function(pvjson) {
                m.startComputation();
      ...
      normalizedPvjsonSource = Rx.Observable.return(kaavio.sourceData.normalizedPvjsonCurrent)
        .toPromise();
    }
    //*/
